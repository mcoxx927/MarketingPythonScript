...
GO
/****** Object:  StoredProcedure [dbo].[GetFinalizedNicheList]    Script Date: 9/3/2025 12:26:18 PM ******/
SET ANSI_NULLS ON
GO
...
--Modify by-   Abhishek        
--modify date -  02-06-2022         
--this procedure will assigning list code and priority for matched and non-matched records.         
--GetFinalizedNicheList 1139
ALTER PROCEDURE [dbo].[GetFinalizedNicheList]     
@uploadLogId int    
AS        
BEGIN 
 ...
 PRINT 'Region id: ' + CAST(@regionId as varchar)   
  ...
  Declare @tblNiche Nvarchar(Max)
 SET @tblNiche='SELECT * into ##temp FROM NicheList'+Convert(varchar(100),@regionId)+' Where UpdateNicheList Is NULL and RegionId='+Convert(Nvarchar(50), @regionId)+''  
 Exec(@tblNiche)

 DECLARE @NichetableName as table
 (
 NicheListId int,
 RegionId int,
 ...
 PropertyLocation Nvarchar(100),
 PrimaryLocationID Nvarchar(50),
 ListType Nvarchar(100),
 ResponseAddress Nvarchar(100),
 ResponseCity Nvarchar(100),
 ResponseState Nvarchar(50),
 ResponseZip Nvarchar(50),
 UpdateNichelist Nvarchar(50)
 )
  INSERT INTO @NichetableName  
 Select NicheListId,RegionId,MailingAddress,MailingCity,MailingState,MailingZip,PropertyLocation,PrimaryLocationID,ListType,ResponseAddress,ResponseCity,ResponseState,ResponseZip,UpdateNichelist  from ##temp
 Truncate table ##temp
 Drop table  ##temp
    
 -- Insert into MasterTable if niche list have Primary location ID and it matchs with any existing one.        
        
 INSERT INTO @Mastertable            
 SELECT masterlist.PrimaryId, masterlist.LocationId, masterlist.PropertyLoaction, masterlist.Code, nc.ListType, masterlist.OwnerId         
 FROM @NichetableName nc          
 INNER JOIN           
    (SELECT own.Address1 AS MailingAddress, loc.LocationId, own.City AS MailCity,own.State AS MailState,own.Zip AS MailZip   ,        
    loc.Address1 AS PropertyLoaction,loc.PrimaryId,loc.City AS LocationCity,  loc.State AS LocationState,         
    loc.Zip AS LocationZip, lp.Code, loc.RegionId, own.OwnerId         
    FROM owner own          
    INNER JOIN location loc ON loc.OwnerId=own.OwnerId         
    INNER JOIN ListPriority lp ON loc.ListPriorityId=lp.ListPriorityId   
	where (loc.InsertedDate >= DATEADD(MM, -6, GETDATE()) or loc.UpdatedDate >= DATEADD(MM, -6, GETDATE()) )
    ) AS masterlist         
 ON  nc.PrimaryLocationId =  masterlist.PrimaryId --AND  nc.PropertyLocation = masterlist.PropertyLoaction         
 WHERE masterlist.RegionId =nc.RegionId AND nc.PrimaryLocationId IS NOT NULL AND nc.PrimaryLocationId != '' 

        
       
 -- Insert into MasterTable if niche list doesn't have Primary location ID and it matchs with any existing one by mailing address.        
 ...
 ON          
 RTRIM(LTRIM(nc.MailingAddress)) = RTRIM(LTRIM(masterlist.MailingAddress))        
 AND         
 RTRIM(LTRIM(nc.PropertyLocation)) = RTRIM(LTRIM(masterlist.PropertyLoaction))        
 WHERE masterlist.RegionId =nc.RegionId AND nc.PrimaryLocationId IS NULL        
        
        
 DECLARE @row int = 1;        
 DECLARE @Listcode VARCHAR(50);        
 DECLARE @LocationPrimaryId VARCHAR(100);        
 DECLARE @NicheListCodeId VARCHAR(50);        
 DECLARE @CombinedListCode NVARCHAR(100);        
 DECLARE @LocationId INT;        
 DECLARE @OwnerId INT;        
 DECLARE @rowss int = (SELECT MAX(RowId) FROM @Mastertable)    
 ...
 BEGIN                
  ...
  --SELECT @Listcode = SUBSTRING(ListCode,0,7) FROM @Mastertable WHERE RowId = @row;            
  SELECT @Listcode = ListCode , @NicheListCodeId = NicheListCode, @LocationId = LocationId, @OwnerId = OwnerId ,@LocationPrimaryId=PrimaryId      
  FROM @Mastertable WHERE RowId = @row;        
  SET @CombinedListCode = @NicheListCodeId + ' - ' + @Listcode;        
        
  -- Check if that list code is already there.        
        
  -- Check if current list code contains ' - ' with spaces.        
  IF @Listcode LIKE '% - %'        
   BEGIN        
    IF @Listcode LIKE '%' + @NicheListCodeId + '%'        
     BEGIN        
      SET @CombinedListCode = @Listcode;        
     END        
   END        
    
       
  IF NOT EXISTS( SELECT 1 FROM ListPriority where Code = @CombinedListCode)            
   BEGIN        
   PRINT 'New List ID'        
    INSERT INTO ListPriority (Code, Name, PriorityLevel, ProcessingOrder, IsAbsentee)         
    VALUES (@CombinedListCode, @CombinedListCode, 0, 0, 2)               
        
    DECLARE @LastInsertedListId Int = SCOPE_IDENTITY();        
    UPDATE Location SET IsPrimaryLocation = 0 WHERE OwnerId=@OwnerId and RegionId=@regionId and (InsertedDate >= DATEADD(MM, -6, GETDATE()) or UpdatedDate >= DATEADD(MM, -6, GETDATE()) )       
    UPDATE Location SET ListPriorityId = @LastInsertedListId, IsPrimaryLocation=1,UpdatedDate=GETDATE()  WHERE LocationId = @LocationId and (InsertedDate >= DATEADD(MM, -6, GETDATE()) or UpdatedDate >= DATEADD(MM, -6, GETDATE()) )      
    --now we are inserting in RegionListFrequency niche list code as 30 days marketing frequency for each region.      
     
  SET @tbl='Update NicheList'+Convert(varchar(100),@regionId)+' Set UpdateNichelist=''Process'' where PrimaryLocationId='''+@LocationPrimaryId+'''';
  Print @tbl
  Exec(@tbl);
	

    DECLARE @i int = 1, @MaxregionId int;        
     SELECT @MaxregionId= MAX(regionid) FROM region        
      WHILE @i <= @MaxregionId        
       BEGIN        
             
        IF EXISTS (SELECT RegionId FROM Region WHERE RegionId=@i)        
         BEGIN        
         --Inserting into RegionListFrequency as default 30 days marketing frequency.        
         INSERT INTO RegionListFrequency VALUES ( 26,@LastInsertedListId,@i,4)           
         END        
         SET @i = @i + 1            
       END             
    END        
  ...
  ELSE        
   BEGIN        
    PRINT 'Old List ID'        
    UPDATE Location SET IsPrimaryLocation = 0,ListPriorityId=(SELECT ListPriorityId FROM ListPriority where Code = @CombinedListCode) WHERE OwnerId=@OwnerId and RegionId=@regionId and (InsertedDate >= DATEADD(MM, -6, GETDATE()) or UpdatedDate >= DATEADD(MM, -6, GETDATE()) )       
    UPDATE Location SET ListPriorityId=(SELECT ListPriorityId FROM ListPriority where Code = @CombinedListCode), IsPrimaryLocation=1 ,UpdatedDate=GETDATE()         
    WHERE LocationId = @LocationId and (InsertedDate >= DATEADD(MM, -6, GETDATE()) or UpdatedDate >= DATEADD(MM, -6, GETDATE()) )     
   SET @tbl='Update NicheList'+Convert(varchar(100),@regionId)+' Set UpdateNichelist=''Process'' where PrimaryLocationId='''+@LocationPrimaryId+'''';
   Print @tbl
  Exec(@tbl);    
   END        
 ...
 IF EXISTS(SELECT * FROM @NichetableName Where UpdateNicheList Is NULL and RegionId=@regionId)    
 BEGIN    
 --New Insert in Databse Location Table ---------------    
----------------------------------------------------------------------------------------------------------------------------------------------                 
  DECLARE @dSql NVARCHAR(MAX) = ''                                
  DECLARE @dSql2 NVARCHAR(MAX) = ''                                
  DECLARE @dSqlTable NVARCHAR(MAX) = ''                               
  DECLARE @dSqlInsert NVARCHAR(MAX) = ''                                
  DECLARE @tableName NVARCHAR(50) = 'CommonNicheList'+Convert(varchar(50),@regionId)  
...
--isnull(ROUND(cast(MAX(rl.LocationSellDate) as decimal(18,2)),2),0) AS LocationSellDate  ,            
 ...
 Max(ss.NCOA) as ResponseNCOA     
  FROM Rawlocations rl                    
   left join TempOwnerNicheListForAccuzip ss                     
   on  rl.RegionRowId = ss.RegionAddressId AND rl.RegionId = ss.RegionId      
    left join @NichetableName nl on rl.LocationPrimaryId=nl.PrimaryLocationId    
 ...
 FROM SmartyStreetNicheLocationAddress where RegionId = @regionId) T                                
 ...
 WHERE                                 
  ...
  AND nl.UpdateNichelist IS NULL                  ---------Not Update records in nichel list    
...
--TRS2 Trust: 5 - 8                                
UPDATE [Location] SET ListPriorityId = 5 WHERE IsTrust = 1 AND RegionId = @regionId and Convert(date,UpdatedDate)!=convert(date,GETDATE()) and (InsertedDate >= DATEADD(MM, -6, GETDATE()) or UpdatedDate >= DATEADD(MM, -6, GETDATE()) )                                                          
--CHURCH Church: 10 - 99                                
UPDATE [Location] SET ListPriorityId = 10 WHERE IsChurch = 1 AND RegionId = @regionId      and Convert(date,UpdatedDate)!=convert(date,GETDATE()) and (InsertedDate >= DATEADD(MM, -6, GETDATE()) or UpdatedDate >= DATEADD(MM, -6, GETDATE()) )                                                      
--INH1 Absentee List 1: 6 - 1                                
UPDATE [Location] SET ListPriorityId = 6 WHERE ListPriorityId = 11 AND IsOwnerOccupied = 0  and Convert(date,UpdatedDate)!=convert(date,GETDATE()) and (InsertedDate >= DATEADD(MM, -6, GETDATE()) or UpdatedDate >= DATEADD(MM, -6, GETDATE()) )                                 
AND OwnerGrantorMatch = 1 AND RegionId = @regionId                      
--OIN1 Owner-Occupant List 1: 1 - 2                                
UPDATE [Location] SET ListPriorityId = 1 WHERE ListPriorityId = 11 AND IsOwnerOccupied = 1   and Convert(date,UpdatedDate)!=convert(date,GETDATE()) and (InsertedDate >= DATEADD(MM, -6, GETDATE()) or UpdatedDate >= DATEADD(MM, -6, GETDATE()) )                                   
AND OwnerGrantorMatch = 1 AND RegionId = @regionId                                
--ABS1 Absentee List 3: 7 - 3                                
UPDATE [Location] SET ListPriorityId = 7 WHERE ListPriorityId = 11 AND IsOwnerOccupied = 0    and Convert(date,UpdatedDate)!=convert(date,GETDATE()) and (InsertedDate >= DATEADD(MM, -6, GETDATE()) or UpdatedDate >= DATEADD(MM, -6, GETDATE()) )                                  
AND (NOT(SellDate IS NULL) AND SellDate <= @regionInputDate1) AND RegionId = @regionId                      
--TRS1 Absentee List 4: 8 - 4                                
UPDATE [Location] SET ListPriorityId = 8 WHERE ListPriorityId = 11 AND IsOwnerOccupied = 0  and Convert(date,UpdatedDate)!=convert(date,GETDATE()) and (InsertedDate >= DATEADD(MM, -6, GETDATE()) or UpdatedDate >= DATEADD(MM, -6, GETDATE()) )                                    
...
UPDATE [Location] SET ListPriorityId = 13 WHERE ListPriorityId = 11 AND IsOwnerOccupied = 1 AND (NOT(SellDate IS NULL) AND SellDate <= CONVERT(date,DATEADD(yy,-20,GETDATE()))) AND RegionId = @regionId    and Convert(date,UpdatedDate)!=convert(date,GETDATE()) and (InsertedDate >= DATEADD(MM, -6, GETDATE()) or UpdatedDate >= DATEADD(MM, -6, GETDATE()) )                                                       
...
UPDATE [Location] SET ListPriorityId = 2 WHERE ListPriorityId = 11 AND IsOwnerOccupied = 1 AND (NOT(SellDate IS NULL) AND SellDate <= CONVERT(date,DATEADD(yy,-13,GETDATE()))) AND RegionId = @regionId  and Convert(date,UpdatedDate)!=convert(date,GETDATE()) and (InsertedDate >= DATEADD(MM, -6, GETDATE()) or UpdatedDate >= DATEADD(MM, -6, GETDATE()) )                            
  
                           
--OON1 Owner-Occupant List 4: 3 - 6                                
UPDATE [Location] SET ListPriorityId = 3 WHERE ListPriorityId = 11 AND IsOwnerOccupied = 1 AND (NOT(SellAmount IS NULL) AND SellAmount <= @regionInputAmount1) AND RegionId = @regionId  and Convert(date,UpdatedDate)!=convert(date,GETDATE())  and (InsertedDate >= DATEADD(MM, -6, GETDATE()) or UpdatedDate >= DATEADD(MM, -6, GETDATE()) )                                         
  
               
--BUY1 Absentee List 5: 9 - 9                                
UPDATE [Location] SET ListPriorityId = 9 WHERE IsOwnerOccupied = 0 AND (NOT(SellAmount IS NULL) AND SellAmount >= @regionInputAmount2) AND (NOT(SellDate IS NULL) AND SellDate >= @regionInputDate2) AND RegionId = @regionId  and Convert(date,UpdatedDate)!=convert(date,GETDATE()) and (InsertedDate >= DATEADD(MM, -6, GETDATE()) or UpdatedDate >= DATEADD(MM, -6, GETDATE()) )                                                         
--BUY2 Owner-Occupant List 5: 4 - 10                                
UPDATE [Location] SET ListPriorityId = 4 WHERE IsOwnerOccupied = 1 AND (NOT(SellAmount IS NULL) AND SellAmount >= @regionInputAmount2) AND (NOT(SellDate IS NULL) AND SellDate >= @regionInputDate2) AND RegionId = @regionId and Convert(date,UpdatedDate)!=convert(date,GETDATE()) and (InsertedDate >= DATEADD(MM, -6, GETDATE()) or UpdatedDate >= DATEADD(MM, -6, GETDATE()) )                                                         
--END LIST PRIORITY--                                
...
UPDATE [Owner] SET                                 
 ...
 WHERE OwnerId IN(SELECT DISTINCT OwnerId FROM [Location] WHERE RegionId = @regionId)                                
UPDATE [Location] SET Address1 = dbo.ProperCase(Address1) WHERE RegionId = @regionId AND (InsertedDate >= DATEADD(MM, -6, GETDATE()) or UpdatedDate >= DATEADD(MM, -6, GETDATE()))                                                                
